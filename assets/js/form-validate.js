	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#AdellaResto-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#AdellaResto-contact').find('input,textarea').attr({value:''});
				jQuery('#AdellaResto-contact').css({opacity:1});
				jQuery('#AdellaResto-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#AdellaResto-contact').animate({opacity:0.3});
		jQuery('#AdellaResto-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#AdellaResto-contact').find('input,textarea').css('border','none');
